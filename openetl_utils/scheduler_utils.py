...
def send_task_to_celery(job_id, job_name, job_type, source_connection, target_connection, source_table, target_table,
    ...
    This function sends a task to Celery to run a pipeline with the specified configurations.
    It logs the task ID and name, and if the task is sent successfully, logs a success message.
    If there is an error sending the task, it logs an error message and raises the exception.

    After sending the task, it updates the integration status in the database to RUNNING,
    and creates a new integration history record with the celery task ID and RUNNING status.
    """
    logger.info(f"Sending task to Celery for job_id: {job_id}, job_name: {job_name}")
    try:
        # Send the task to Celery with the specified args and kwargs
        ...
        # Log an error message if there is an error sending the task
    ...
    def update_db():
        """
        Update the integration status in the database to RUNNING,
        and create a new integration history record with the celery task ID and RUNNING status.
        """
        try:
            # Update the integration record
            db.update_integration(record_id=job_id, last_run=datetime.utcnow(), is_running=True)
            # Create a new integration history record
    ...
    # Call the update_db function with retries
    update_db()
...
def check_and_schedule_tasks():
    ...
    This function checks the database for new integrations and schedules tasks using APScheduler.
    It also removes jobs not in the database and disables jobs for disabled integrations.
    """
...
def clean_up_old_logs():
    ...
    This function is run periodically (default: every 60 seconds) to remove old log files.
    """
    ...
    try:
        ...
        # Log any errors that occur while cleaning up old logs
...
def start_scheduler():
    ...
    This function starts the APScheduler, removes all existing jobs, and adds two periodic jobs:

    1. A job that checks and schedules tasks every 60 seconds.
    2. A job that cleans up old log files every 24 hours.

    The function also logs a message when the scheduler is started and when it is shut down.
    """
    # Remove all existing jobs
    scheduler.remove_all_jobs()

    # Add a periodic job to check and schedule tasks every 60 seconds
    scheduler.add_job(
    ...
    # Add a periodic job to clean up old log files every 24 hours
    scheduler.add_job(
